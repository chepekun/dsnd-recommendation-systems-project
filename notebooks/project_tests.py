        ...
        for k, v in sol_1_dict.items():
            if sol_1_dict_[k] != sol_1_dict[k]:
                print(
                    ...
                    " be a string.  Let each row be considered a separate user-article interaction. "
                    ...
                    " However, this is hard to catch without mapping first!".format(k)
                )
    ...
    if sol_5_dict == sol_5_dict_1:
        ...
        for k, v in sol_5_dict_1.items():
            if sol_5_dict[k] != sol_5_dict_1[k]:
                print(
                    "Oops!  Looks like there is a mistake with the {} key in your dictionary. "
                    " The answer should be {}.  Try again.".format(k, v)
                )
...
def sol_4_test(sol_4_dict):
    ...
    sol_4_dict_1 = {
        ...
        "How many articles in the test set are we not able to make"
        " predictions for because of the cold start problem?": d,
    }

    if sol_4_dict == sol_4_dict_1:
        print(
            "Awesome job!  That's right!  All of the test articles are in the training data, but there "
            " are only 20 test users that were also in the training set.  All of the other users that are "
            " in the test set we have no data on.  Therefore, we cannot make predictions for these users using SVD."
        )
    else:
